...
class RecordTask(Task):
    ...
    def run(self):
        ...
        if self.estimated_duration_s < 5:
            ...
            self.control.window.tem_tasks.rotation_button.setText("Rotation")
            self.control.window.tem_tasks.rotation_button.started = False
            return
        ...
        self.rotations_angles = []
        stage_moving = False
        while self.control.tem_status['stage.GetPos'][3] < phi1 and self.control.window.tem_tasks.rotation_button.started:
            time.sleep(0.5)
            ...
            if int(self.control.tem_status['stage.GetStatus'][3]) != 1 and stage_moving:
                ...
                if self.writer and self.control.window.file_operations.streamWriterButton.started:
                    self.writer()
                break
        ...
        if self.control.window.tem_tasks.autoreset_checkbox.isChecked(): 
        ...
        if self.writer and os.path.isfile(self.control.window.file_operations.formatted_filename):
            self.control.temtools.addinfo_to_hdf()
