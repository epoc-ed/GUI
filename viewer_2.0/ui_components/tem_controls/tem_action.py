...
class TEMAction(QObject):
    ...
    def callGetInfoTask(self):
        if self.tem_tasks.gettem_checkbox.isChecked() and not os.access(self.file_operations.outPath_input.text(), os.W_OK):
        ...
        if self.tem_tasks.gettem_checkbox.isChecked():
            prefix = self.file_operations.prefix_input.text().strip()
            self.formatted_filename = self.file_operations.generate_h5_filename(prefix)
            self.control.trigger_getteminfo.emit('Y')
        else:
            self.control.trigger_getteminfo.emit('N')
            if os.path.isfile(self.file_operations.formatted_filename):
