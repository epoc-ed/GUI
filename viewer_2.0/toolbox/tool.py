    ...
    def addinfo_to_hdf(self):
        with h5py.File(self.filename, 'a') as f:
            try:
                ...
                # f.create_dataset('dentry/instrument/detector/detector_distance', data = data_shape[0], dtype='uint64') <- LUT
                ...
                # f.create_dataset('entry/instrument/optics/CL_size', data = 'XX um') # <- LUT
                f.create_dataset('entry/instrument/optics/SA_ID', data = self.tem_status['apt.GetSize(4)'], dtype='uint8')
                # f.create_dataset('entry/instrument/optics/SA_size', data = 'XX um') # <- LUT
