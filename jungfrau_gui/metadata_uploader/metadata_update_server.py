...
VERSION = "for JF_GUI/v2025.12.15 or later"
V_DATE = "2025.12.15"
...
class Hdf5MetadataUpdater:
    def __init__(self, port_number = 3463):
        ...
        self.root_data_directory = "/data/epoc/storage/jem2100plus/" # self.cfg.base_data_dir.as_posix()
        self.results = None
        self.error_retry = 5
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                if not 'tem_status' in message: continue

                filename = self.root_data_directory + message["filename"]
                logging.info(f"Updating metadata of {filename}...")
                beam_property = message["beam_property"]                    
                rotations_angles=message["rotations_angles"]
                image_encoded = message["image_supporting"]
                if image_encoded is None:
                    images_supporting = None
                else:
                    images_received = pickle.loads(zlib.decompress(base64.b64decode(image_encoded)))
                ...
                self.addinfo_to_hdf(
                    ...
                    images_supporting=images_supporting,
    ...
    def addinfo_to_hdf(self, filename, tem_status, beam_property, detector_distance, aperture_size_cl, aperture_size_sa, rotations_angles, jf_threshold, jf_gui_tag, commit_hash, textnote="", images_supporting=None, pixel=0.075):
        ...
        try:
            with h5py.File(filename, 'a') as f:
                try:
                    ...
                    create_or_update_dataset('entry/instrument/detector/beam_center_x', data = beamcenter[0], dtype='int')
                    create_or_update_dataset('entry/instrument/detector/beam_center_y', data = beamcenter[1], dtype='int')
                    create_or_update_dataset('entry/instrument/detector/detector_distance', data = detector_distance, dtype='uint64')
                    ...
                    create_or_update_dataset('entry/cif/_diffrn_source_voltage', data = f'{ht:3.0f}')
                    ...
                    create_or_update_dataset('entry/freetext', data = textnote)
    ...
    if args.version:
        print('''
            **Detailed information of authors, years, project name, Github URL, license, contact address, etc.**
            Metadata-update-server for Electron Diffraction with JUNGFRAU (2024-2025)
            https://github.com/epoc-ed/GUI
            EPOC Project (2024-2025)
