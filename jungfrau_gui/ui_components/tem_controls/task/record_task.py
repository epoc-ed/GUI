...
class RecordTask(Task):
    ...
    def run(self):
        ...
        try:
            ...
            # Parameters to make premature stop detection more robust
            angle_tolerance_deg = 0.5
            max_false_stop_checks = 10   # 10 * 0.1 s ~= 1 second
            false_stop_checks = 0
            ...
            try:
                self.client.SetTiltXAngle(phi_target)
            except Exception as e:
                logging.error(f"Unexpected error while sending SetTiltXAngle: {e}")
                self.client.SetBeamBlank(1)
                logging.warning("Beam blanked to protect sample!")
                if not self.client.is_rotating:
                    stop_reason = "set_tilt_failed_and_not_rotating"
                    return
                else:
            ...
            try:
                ...
                stop_reason = f"rotation_start_failed: {rotation_error}"
            ...
            try:
                while True:
                    try:
                        ...
                        t = time.time()
                        current_angle = pos[3]
                        delta = phi_target - current_angle

                        log_dir = os.path.dirname(self.log_suffix)
                        can_write_log = (log_dir == "") or os.access(log_dir, os.W_OK)

                        if can_write_log:
                            logfile.write(f"{t - t0:10.6f}  {current_angle:8.3f} deg\n")

                        logging.info(
                            f"{t - t0:10.6f}  {current_angle:8.3f} deg  "
                            f"(target={phi_target:8.3f}, delta={delta:7.3f}, rotating={self.client.is_rotating})"
                        )
                        self.rotations_angles.append([f"{t - t0:10.6f}", f"{current_angle:8.3f}"])

                        # Normal stop condition: stage reached target angle
                        if abs(delta) <= angle_tolerance_deg:
                            stop_reason = "target_reached"
                            logging.warning(
                                f"Stopping acquisition: reached target within {angle_tolerance_deg} deg"
                            )
                            break

                        # Suspicious early stop: confirm it for ~1 second before stopping
                        if not self.client.is_rotating:
                            false_stop_checks += 1
                            logging.warning(
                                f"is_rotating=False before target "
                                f"(current={current_angle:.3f}, target={phi_target:.3f}, delta={delta:.3f}) "
                                f"[{false_stop_checks}/{max_false_stop_checks}]"
                            )
                            if false_stop_checks >= max_false_stop_checks:
                                stop_reason = "rotation_flag_false_confirmed"
                                logging.warning("Rotation stop confirmed by repeated checks.")
                                break
                        else:
                            false_stop_checks = 0
                        ...
                        false_stop_checks += 1
                        if false_stop_checks >= max_false_stop_checks:
                            stop_reason = f"too_many_position_errors: {e}"
                            logging.error("Too many consecutive stage-position errors, stopping acquisition.")
                            break
                        time.sleep(0.1)
                        continue

            except TimeoutError as te:
                stop_reason = f"timeout: {te}"
                logging.error(f"TimeoutError during rotation: {te}")
            except Exception as e:
                stop_reason = f"unexpected_rotation_error: {e}"
                logging.error(f"Unexpected error caught for TEMClient::is_rotating(): {e}")

            logging.warning(f"Acquisition stop reason: {stop_reason}")

            if self.writer is not None:
                logging.info(" ********************  Stopping Data Collection...")
                logging.warning("Writer stop requested.")
                self.writer[1]()

            time.sleep(0.01)
            logging.warning("Beam blank ON.")
            self.client.SetBeamBlank(1)

            try:
                phi_final = self.client.GetTiltXAngle()
                log_dir = os.path.dirname(self.log_suffix)
                can_write_log = (log_dir == "") or os.access(log_dir, os.W_OK)
                if can_write_log:
                    logfile.write(f"# Final Angle (measured):   {phi_final:.3f} deg\n")
            except Exception as e:
                phi_final = None
            ...
            if phi_final is not None:
                logging.info(f"Stage rotation end at {phi_final:.1f} deg.")
            else:
                logging.info("Stage rotation ended, but final angle could not be read.")

            try:
                self.control.send_to_tem("#more", asynchronous=False)
                logging.info('TEM-status received.')
            except Exception as e:
                logging.error(f"Error updating TEM status: {e}")
            ...
            if not self.tem_action.tem_tasks.connecttem_button.started:
                QMetaObject.invokeMethod(
                    self.tem_action.tem_tasks.connecttem_button,
                    "click",
                    Qt.QueuedConnection
                )
            ...
            logging.warning("Beam blank ON in finally cleanup.")
            self.client.SetBeamBlank(1)
            time.sleep(0.01)
